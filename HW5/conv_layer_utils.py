...
def conv_bn_relu_pool_forward(x, w, b, conv_param, gamma, beta, bn_param, pool_param):
  a, conv_cache = conv_forward_fast(x, w, b, conv_param)
  b, bn_cache = spatial_batchnorm_forward(a, gamma, beta, bn_param) #adding batch norm forward pass
...
def conv_bn_relu_pool_backward(dout, cache):
  ...
  da, dgamma, dbeta = spatial_batchnorm_backward(db, bn_cache) # batch norm backward pass
  dx, dw, db = conv_backward_fast(da, conv_cache)
