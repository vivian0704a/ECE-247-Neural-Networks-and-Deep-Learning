...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax_loss= np.sum(-np.log(np.exp(scores[np.arange(N), y])/np.sum(np.exp(scores), axis=1)))/N
    regular_loss= 0.5*reg*(np.linalg.norm(W1)**2 + np.linalg.norm(W2)**2)
    loss= softmax_loss + regular_loss
  ...
  def predict(self, X):
    ...
    relu= lambda x: x * (x>0)
    h1_r=relu(h1)
    scores=np.dot(h1_r, self.params['W2'].T) + self.params['b2']
