...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_normalize = (x-x_mean)/(x_var + eps)**0.5
        out = gamma*x_normalize + beta

        running_mean = momentum * running_mean + (1 - momentum) * x_mean
        running_var = momentum * running_var + (1 - momentum) * x_var
        bn_param['running_mean'] = running_mean
        bn_param['runnung_var'] = running_var

        cache={}
        cache['x_mean'] = x_mean
        cache['x_var'] = x_var
        cache['x_normalize'] = x_normalize
        cache['gamma'] = gamma
        #cache['beta'] = beta
        cache['eps'] = eps
        cache['x'] = x
        ...
        test_normalize = (x-running_mean)/(running_var + eps)**0.5
...
def batchnorm_backward(dout, cache):
    ...
    dx_n = dout * gamma
    da = dx_n/(x_var + eps)**0.5
    db = dx_n*(x-x_mean)
    dc = -db/(x_var+eps)
    dvar = (0.5*dc/(x_var+eps)**0.5).sum(axis=0)
    dmu = 2 *(x-x_mean)*dvar *np.ones_like(dout)/dout.shape[0]
    dx =  da + dmu - np.sum((da+dmu), axis=0)*np.ones_like(dout)/dout.shape[0]
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        # From coding discussion: we use <p to simulate the probability of *keeping* each neuron output.
