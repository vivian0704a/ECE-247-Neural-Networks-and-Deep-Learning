...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss, dscores = softmax_loss(scores, y)
        weight=[self.params['W' + str(i+1)] for i in range(self.num_layers)]
        loss += 0.5 * self.reg* sum(np.linalg.norm(j)**2 for j in weight)

        for i in range(self.num_layers-1, -1, -1):
          if i==self.num_layers-1:
            dout, dw, db=affine_backward(dscores, caches[i+1])
            ...
            if self.use_batchnorm:          
              dout, dw, db, dgamma, dbeta=affine_batchnorm_relu_backward(dout, caches[i+1])
